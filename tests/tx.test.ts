...
test('fail to deserialize unknown tx version', (): void => {
  expect((): void => {
    TxVariant.deserialize(ByteBuffer.wrap(new Uint8Array([0xff, 0xff])));
  }).toThrowError('unknown tx version: ' + 0xffff);
});

test('fail to deserialize unknown tx type', (): void => {
  expect((): void => {
    TxVariant.deserialize(ByteBuffer.wrap(new Uint8Array([0x00, 0x00, 0xff])));
